    ...
    while True:
      ...
      if not self.stock:
        ...
        for i, item in enumerate(self.stock, 1):
          print(f"{i}. {item.name} ({item.description}) - {item.value} gold")
      ...
      # Ensure player has an inventory method implemented
      if hasattr(player, "add_item"):
        player.add_item(item)

      # Optional: Remove item so it can only be bought once
      self.stock.pop(choice - 1)
